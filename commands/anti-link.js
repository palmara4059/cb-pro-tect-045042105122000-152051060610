...
const schema = require('../schema/anti-link');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('anti-link')
        .setDescription(`Permet d'activer ou de désactiver l'anti-link !`)
    ...
    async execute(client, interaction) {
        ...
        if (!guild) {
            ...
            if (interaction.options.getString('etat') === 'on') {
                ...
                .setColor('#ffffff')
                .setDescription(`✅ l'anti-link à été activer par ${interaction.user.username} !`)
            return interaction.reply({ embeds: [onStat] })
            } else {
                const onStat = new MessageEmbed()
                .setColor('#ffffff')
                .setDescription(`✅ l'anti-link à été désactiver par ${interaction.user.username} !`)
                return interaction.reply({ embeds: [onStat] })
            }
        }
